...
def Out2File(dict):
    ...
    '''
    with open('TTBT3.csv', 'a') as f:
        field_name = ['Title', 'Content', 'ReplyNum']
        writer = csv.DictWriter(f, fieldnames=field_name)
        writer.writeheader()
        for comment in dict:
            writer.writerow({'Title': comment['title'], 'Content': comment['content'], 'ReplyNum': comment['replyNum']})

        print('当前页面爬取完成')

def main(base_url, deep):
    url_list = []
    for i in range(60, deep):
